(function(){var P$=Clazz.newPackage("jalview.structures.models"),p$1={},I$=[[0,'jalview.util.Comparison','StringBuilder','Error','jalview.util.MessageManager','java.util.ArrayList','jalview.datamodel.SequenceI','jalview.datamodel.PDBEntry','java.util.Arrays']],$I$=function(i){return I$[i]||(I$[i]=Clazz.load(I$[0][i]))};
var C$=Clazz.newClass(P$, "AAStructureBindingModel", function(){
...
}var title=Clazz.new_($I$(2).c$$I,[64]);
...
throw Clazz.new_($I$(3).c$$S,[$I$(4).formatMessage$S$OA("error.implementation_error_no_pdbentry_from_index", Clazz.array(java.lang.Object, -1, [Integer.valueOf$I(pe).toString()]))]);
...
s.add$TE(this.getSequence$()[pe][i]);
if (this.getChains$()[pe] != null ) {
if (i < this.getChains$()[pe].length) {
c.add$TE(this.getChains$()[pe][i]);
} else {
c.add$TE("TheNullChain");
}} else {
...
c.add$TE(tchain[i] == null  ? "TheNullChain" : tchain[i]);
}}}
var tmp=s.toArray$TTA(Clazz.array($I$(6), [s.size$()]));
this.getSequence$()[pe]=tmp;
if (c.size$() > 0) {
var tch=c.toArray$TTA(Clazz.array(String, [c.size$()]));
...
;
(function(){var C$=Clazz.newClass(P$.AAStructureBindingModel, "SuperposeData", function(){
