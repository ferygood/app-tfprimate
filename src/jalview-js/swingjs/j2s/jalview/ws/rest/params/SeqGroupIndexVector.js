...
gl.add$I$TE(0, Clazz.array(Integer.TYPE, -1, [0, lowest - 2]));
}if ((al.getHeight$() - 1 - highest ) > this.minsize) {
gl.add$TE(Clazz.array(Integer.TYPE, -1, [highest + 1, al.getHeight$() - 1]));
}} else {
gl.add$TE(Clazz.array(Integer.TYPE, -1, [0, al.getHeight$() - 1]));
...
}var vals=gl.toArray$TTA(Clazz.array(Integer.TYPE, [gl.size$(), null]));
...
return Clazz.new_($I$(7).c$$S,[idvector.toString()]);
});
...
prms.add$TE("minsize='" + this.minsize + "'" );
...
try {
this.minsize=(Integer.valueOf$S(val)).intValue$();
if (this.minsize >= 0) {
return true;
}} catch (x) {
...
lst.add$TE(Clazz.new_($I$(9).c$$S$S$Z$S$S$java_util_Collection$java_net_URL,["sep", "Separator character between elements of vector", true, ",", this.sep, $I$(10).asList$TTA(Clazz.array(String, -1, [" ", ",", ";", "\t", "|"])), null]));
lst.add$TE(Clazz.new_($I$(11).c$$S$S$Z$I$I$I$I,["minsize", "Minimum size of partition allowed by service", true, 1, this.minsize, 1, 0]));
lst.add$TE(this.createMolTypeOption$S$S$Z$jalview_ws_rest_InputType_molType$jalview_ws_rest_InputType_molType("type", "Sequence type", false, this.type, $I$(8).MIX));
return lst;
});
})();
