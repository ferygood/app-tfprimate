(function(){var P$=Clazz.newPackage("jalview.ws.dbsources"),I$=[[0,'com.stevesoft.pat.Regex','jalview.structure.StructureImportSettings','jalview.ws.ebi.EBIFetchClient',['jalview.datamodel.PDBEntry','.Type'],'jalview.io.FileFormat','jalview.io.FormatAdapter','jalview.io.DataSourceType','java.util.ArrayList','jalview.datamodel.DBRefEntry','jalview.util.MessageManager','jalview.io.PDBFeatureSettings']],$I$=function(i){return I$[i]||(I$[i]=Clazz.load(I$[0][i]))};
var C$=Clazz.newClass(P$, "Pdb", null, 'jalview.ws.dbsources.EbiFileRetrievedProxy');
...
return Clazz.new_($I$(1).c$$S,["([1-9][0-9A-Za-z]{3}):?([ _A-Za-z0-9]?)"]);
});
...
pdbAlignment=Clazz.new_($I$(6)).readFile$java_io_File$jalview_io_DataSourceType$jalview_io_FileFormatI(tmpFile, $I$(7).FILE, fileFormat);
if (pdbAlignment != null ) {
var toremove=Clazz.new_($I$(8));
...
var dbentry=Clazz.new_($I$(9).c$$S$S$S,[this.getDbSource$(), this.getDbVersion$(), (chid == null  ? id : id + chid)]);
pdbcs.addDBRef$jalview_datamodel_DBRefEntry(dbentry);
} else {
toremove.add$TE(pdbcs);
}}
