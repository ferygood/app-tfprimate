...
var C$=Clazz.newClass(P$, "JabaParamStore", null, null, 'jalview.ws.params.ParamDatastoreI');
...
this.editedParams=Clazz.new_($I$(1));
}, 1);
...
this.editedParams.put$TK$TV(paramset.getName$(), paramset);
} else {
...
var prefs=Clazz.new_($I$(2));
if (this.servicePresets == null ) {
this.servicePresets=Clazz.new_($I$(1));
var prman;
if ((prman=this.service.getPresets$()) != null ) {
var pset=prman.getPresets$();
if (pset != null ) {
for (var pr, $pr = pset.iterator$(); $pr.hasNext$()&&((pr=($pr.next$())),1);) {
var prset=Clazz.new_($I$(3).c$$jalview_ws_jws2_jabaws2_Jws2Instance$compbio_metadata_Preset,[this.service, pr]);
this.servicePresets.put$TK$TV(prset.getName$(), prset);
}
...
var rgs=Clazz.new_($I$(2));
var rgnames=Clazz.new_($I$(2));
for (var rg, $rg = jabargs.iterator$(); $rg.hasNext$()&&((rg=($rg.next$())),1);) {
var narg=null;
var nargstring=null;
if (Clazz.instanceOf(rg, "compbio.metadata.Parameter")) {
narg=Clazz.new_($I$(4).c$$compbio_metadata_Parameter,[rg]);
nargstring=narg.getName$();
} else if (Clazz.instanceOf(rg, "compbio.metadata.Option")) {
narg=Clazz.new_($I$(5).c$$compbio_metadata_Option,[rg]);
nargstring=(rg).getOptionNames$().get$I(0);
}if (narg == null ) {
throw Clazz.new_($I$(6).c$$S,[$I$(7).formatMessage$S$SA("error.implementation_error_cannot_handle_jaba_param", Clazz.array(String, -1, [rg.getClass$().toString()]))]);
} else {
rgs.add$TE(narg);
rgnames.add$TE(nargstring);
}}
if (!sortByOpt) {
return rgs;
}var rgssort=rgs.toArray$TTA(Clazz.array($I$(8), [rgs.size$()]));
var rgssorton=rgnames.toArray$TTA(Clazz.array(String, [rgs.size$()]));
...
return $I$(10).asList$TTA(rgssort);
}, 1);
...
throw Clazz.new_($I$(6).c$$S,[$I$(7).getString$S("error.implementation_error_attempt_to_delete_service_preset")]);
}});

Clazz.newMeth(C$, 'storePreset$S$S$java_util_List', function (presetName, text, jobParams) {
var jps=Clazz.new_($I$(11).c$$S$S$java_util_List,[presetName, text, jobParams]);
jps.setApplicableUrls$SA(Clazz.array(String, -1, [this.service.getUri$()]));
this.editedParams.put$TK$TV(jps.getName$(), jps);
if (this.manager != null ) {
...
if (jps == null ) {
throw Clazz.new_($I$(6).c$$S,[$I$(7).formatMessage$S$SA("error.implementation_error_cannot_locate_oldname_presetname", Clazz.array(String, -1, [oldName, presetName]))]);
...
}var wsp=Clazz.new_($I$(11));
wsp.setName$S(name);
wsp.setDescription$S(descr);
wsp.setApplicableUrls$SA(urls.clone$());
var lines=Clazz.new_($I$(2));
var st=Clazz.new_($I$(12).c$$S$S,[parameterfile, "\n"]);
while (st.hasMoreTokens$()){
lines.add$TE(st.nextToken$());
}
...
throw Clazz.new_($I$(6).c$$S,[$I$(7).getString$S("error.implementation_error_jabaws_param_set_only_handled_by")]);
}var rslt=Clazz.new_($I$(14));
