(function(){var P$=Clazz.newPackage("jalview.io"),p$1={},I$=[[0,'com.stevesoft.pat.Regex','java.util.Hashtable','java.io.FileReader','java.io.BufferedReader','fr.orsay.lri.varna.factories.RNAFactory','jalview.datamodel.SequenceI','jalview.datamodel.Sequence','jalview.datamodel.Annotation','jalview.analysis.Rna','jalview.datamodel.AlignmentAnnotation','StringBuffer','java.util.LinkedHashMap','jalview.util.MessageManager','jalview.util.DBRefUtils','jalview.datamodel.SequenceFeature','java.util.Vector','java.util.ArrayList','jalview.util.MapList','jalview.datamodel.Mapping','jalview.schemes.ResidueProperties','jalview.util.Comparison','jalview.util.Format']],$I$=function(i){return I$[i]||(I$[i]=Clazz.load(I$[0][i]))};
var C$=Clazz.newClass(P$, "StockholmFile", null, 'jalview.io.AlignFile');
C$.DETECT_BRACKETS=null;
C$.NOT_RNASS=null;
C$.typeIds=null;

C$.$clinit$ = function() {Clazz.load(C$, 1);
C$.DETECT_BRACKETS=Clazz.new_($I$(1).c$$S,["(<|>|\\[|\\]|\\(|\\)|\\{|\\})"]);
C$.NOT_RNASS=Clazz.new_($I$(1).c$$S,["^[^<>[\\](){}A-DF-Za-df-z]*$"]);
C$.typeIds=null;
{
if (C$.typeIds == null ) {
C$.typeIds=Clazz.new_($I$(2));
C$.typeIds.put$TK$TV("SS", "Secondary Structure");
C$.typeIds.put$TK$TV("SA", "Surface Accessibility");
C$.typeIds.put$TK$TV("TM", "transmembrane");
C$.typeIds.put$TK$TV("PP", "Posterior Probability");
C$.typeIds.put$TK$TV("LI", "ligand binding");
C$.typeIds.put$TK$TV("AS", "active site");
C$.typeIds.put$TK$TV("IN", "intron");
C$.typeIds.put$TK$TV("IR", "interacting residue");
C$.typeIds.put$TK$TV("AC", "accession");
C$.typeIds.put$TK$TV("OS", "organism");
C$.typeIds.put$TK$TV("CL", "class");
C$.typeIds.put$TK$TV("DE", "description");
C$.typeIds.put$TK$TV("DR", "reference");
C$.typeIds.put$TK$TV("LO", "look");
C$.typeIds.put$TK$TV("RF", "Reference Positions");
}};
}
...
var fr=null;
fr=Clazz.new_($I$(3).c$$java_io_File,[inFile]);
var r=Clazz.new_($I$(4).c$$java_io_Reader,[fr]);
var result=null;
try {
...
seqs[i]=Clazz.new_($I$(7).c$$S$S$I$I,[id, seq, begin, end]);
...
ann[k]=Clazz.new_($I$(8).c$$S$S$C$F,[annot[k], "", $I$(9).getRNASecStrucState$S(annot[k]).charAt$I(0), 0.0]);
}
var align=Clazz.new_($I$(10).c$$S$S$jalview_datamodel_AnnotationA,["Sec. str.", current.getID$(), ann]);
...
this.annotations.addElement$TE(align);
}
this.setSeqs$jalview_datamodel_SequenceIA(seqs);
});

Clazz.newMeth(C$, 'parse$', function () {
var treeString=Clazz.new_($I$(11));
var treeName=null;
var line;
var version;
var seqAnn=Clazz.new_($I$(2));
var seqs=Clazz.new_($I$(12));
var p;
var r;
var rend;
var s;
var x;
r=Clazz.new_($I$(1).c$$S,["# STOCKHOLM ([\\d\\.]+)"]);
...
}rend=Clazz.new_($I$(1).c$$S,["^\\s*\\/\\/"]);
p=Clazz.new_($I$(1).c$$S,["(\\S+)\\/(\\d+)\\-(\\d+)"]);
s=Clazz.new_($I$(1).c$$S,["(\\S+)\\s+(\\S*)\\s+(.*)"]);
r=Clazz.new_($I$(1).c$$S,["#=(G[FSRC]?)\\s+(.*)"]);
x=Clazz.new_($I$(1).c$$S,["(\\S+)\\s+(\\S+)"]);
var openparen=Clazz.new_($I$(1).c$$S$S,["(<|\\[)", "("]);
var closeparen=Clazz.new_($I$(1).c$$S$S,["(>|\\])", ")"]);
rend.optimize$();
p.optimize$();
s.optimize$();
r.optimize$();
...
var dbsource=null;
var pf=Clazz.new_($I$(1).c$$S,["PF[0-9]{5}(.*)"]);
var rf=Clazz.new_($I$(1).c$$S,["RF[0-9]{5}(.*)"]);
...
}var seqO=Clazz.new_($I$(7).c$$S$S$I$I,[sid, seq, start, end]);
...
var feat=Clazz.new_($I$(15).c$$S$S$I$I$S,[type, desc, new_pos, new_pos, null]);
seqO.addSequenceFeature$jalview_datamodel_SequenceFeature(feat);
}}
}
}
}this.seqs.addElement$TE(seqO);
}
return;
...
ann=Clazz.new_($I$(2));
}ann.put$TK$TV(type, content);
seqAnn.put$TK$TV(acc, ann);
} else {
...
features=Clazz.new_($I$(2));
ann.put$TK$TV("features", features);
}var content;
if (features.containsKey$O(this.id2type$S(type))) {
content=features.get$O(this.id2type$S(type));
} else {
content=Clazz.new_($I$(2));
features.put$TK$TV(this.id2type$S(type), content);
}var ns=content.get$O("annotation");
if (ns == null ) {
ns="";
}ns += seq;
content.put$TK$TV("annotation", ns);
var strucAnn;
if (seqAnn.containsKey$O(acc)) {
strucAnn=seqAnn.get$O(acc);
} else {
strucAnn=Clazz.new_($I$(2));
}var newStruc=Clazz.new_($I$(16));
...
strucAnn.put$TK$TV(type, newStruc);
seqAnn.put$TK$TV(acc, strucAnn);
} else {
...
var mp=Clazz.new_($I$(18).c$$IA$IA$I$I,[Clazz.array(Integer.TYPE, -1, [seqO.getStart$(), seqO.getEnd$()]), Clazz.array(Integer.TYPE, -1, [st, en]), 1, 1]);
var mping=Clazz.new_($I$(19).c$$jalview_util_MapList,[mp]);
...
var ann;
ann=Clazz.new_($I$(8).c$$S$S$C$F,[pos, "", " ", 0.0]);
if (ss) {
{
...
if (annot == null ) {
annot=Clazz.new_($I$(10).c$$S$S$jalview_datamodel_AnnotationA,[type, type, els]);
annotation.addElement$TE(annot);
} else {
...
if (dataRef == null ) {
dataRef=Clazz.new_($I$(2));
}for (var idb=0; idb < ndb; idb++) {
var ref=seqrefs.get$I(idb);
var datAs1=ref.getSource$().toString() + " ; " + ref.getAccessionId$().toString() ;
dataRef.put$TK$TV(tmp, datAs1);
}
}$in++;
}
maxid+=9;
...
this.out.append$S(Clazz.new_($I$(22).c$$S,["%-" + (maxid - 2) + "s" ]).form$S("#=GS " + idd.toString() + " " ));
if (type.contains$CharSequence("PFAM") || type.contains$CharSequence("RFAM") ) {
...
continue;
}this.out.append$S(Clazz.new_($I$(22).c$$S,["%-" + maxid + "s" ]).form$S("#=GR " + this.printId$jalview_datamodel_SequenceI$Z(seq, jvSuffix) + " " + key + " " ));
...
}this.out.append$S(Clazz.new_($I$(22).c$$S,["%-" + maxid + "s" ]).form$S(this.printId$jalview_datamodel_SequenceI$Z(seq, jvSuffix) + " "));
...
key=C$.type2id$S(aa.label.toLowerCase$());
if (key == null ) {
label=aa.label;
} else {
label=key + "_cons";
...
this.out.append$S(Clazz.new_($I$(22).c$$S,["%-" + maxid + "s" ]).form$S("#=GC " + label + " " ));
