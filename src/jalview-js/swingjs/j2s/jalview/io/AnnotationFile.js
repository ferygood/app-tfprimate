...
hasLabels|=(row.annotations[j].displayCharacter != null  && row.annotations[j].displayCharacter.length$() > 0  && !row.annotations[j].displayCharacter.equals$O(" ") );
hasGlyphs|=((row.annotations[j].secondaryStructure).$c() != 0  && row.annotations[j].secondaryStructure != " " );
hasValues|=(!Float.isNaN$F(row.annotations[j].value));
...
if (row.annotations[j].colour != null  && row.annotations[j].colour !== $I$(7).black  ) {
this.text.append$S(comma + "[" + $I$(5).getHexString$java_awt_Color(row.annotations[j].colour) + "]" );
comma=",";
}}this.text.append$S("|");
}
if (row.hasScore$()) {
...
if (color != null  && color !== $I$(7).black   && oneColour ) {
colours.append$S("COLOUR\t");
colours.append$S(row.label);
colours.append$S("\t");
colours.append$S($I$(5).getHexString$java_awt_Color(color));
...
this.text.append$S("\t");
if (sg.textColour !== $I$(7).black ) {
this.text.append$S("textCol1=");
this.text.append$S($I$(5).getHexString$java_awt_Color(sg.textColour));
this.text.append$S("\t");
}if (sg.textColour2 !== $I$(7).white ) {
this.text.append$S("textCol2=");
this.text.append$S($I$(5).getHexString$java_awt_Color(sg.textColour2));
this.text.append$S("\t");
...
this.text.append$S($I$(5).getHexString$java_awt_Color(sg.idColour));
this.text.append$S("\t");
}if (sg.isHidereps$()) {
...
System.out.println$S("Last read line " + this.nlinesread + ": '" + this.lastread + "' (first 80 chars) ..." );
}return false;
} else {
throw ex;
}
...
var sg=null;
for (var _sg, $_sg = al.getGroups$().iterator$(); $_sg.hasNext$()&&((_sg=($_sg.next$())),1);) {
if ((sg=_sg).getName$().equals$O(name)) {
break;
} else {
sg=null;
}}
...
sg.cs.setColourScheme$jalview_schemes_ColourSchemeI($I$(8).getColourScheme$jalview_api_AlignViewportI$jalview_datamodel_AnnotatedCollectionI$S(null, al, value));
