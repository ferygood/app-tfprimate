...
var C$=Clazz.newClass(P$, "HighlightRegionAnnotation", null, null, 'java.io.Serializable');
C$.DEFAULT_OUTLINE_COLOR=null;
C$.DEFAULT_FILL_COLOR=null;
C$.XML_ELEMENT_NAME=null;
C$.XML_VAR_OUTLINE_NAME=null;
C$.XML_VAR_FILL_NAME=null;
C$.XML_VAR_RADIUS_NAME=null;
...
C$.c$$java_util_ArrayList.apply(this, [Clazz.new_($I$(4))]);
}, 1);
...
return Clazz.new_($I$(5).c$$D$D,[2.0 * center.x - p.x, 2.0 * center.y - p.y]);
}, p$1);
...
var m=Clazz.new_($I$(5).c$$D$D,[(p1.x + p2.x) / 2.0, (p1.y + p2.y) / 2.0]);
var rad=p1.distance$java_awt_geom_Point2D(p2) / 2.0;
var angle=Math.atan2(p1.y - m.y, p1.x - m.x);
var incr=0.18479956785822313;
var pdir=Clazz.new_($I$(5).c$$D$D,[m.x + rad * Math.cos(angle + 1.5707963267948966), m.y + rad * Math.sin(angle + 1.5707963267948966)]);
...
var interForward=Clazz.new_($I$(5).c$$D$D,[m.x + rad * Math.cos(angle2), m.y + rad * Math.sin(angle2)]);
result.addLast$TE(interForward);
}
return result;
}, p$1);
...
var p=Clazz.new_($I$(7));
var pointList=Clazz.new_($I$(6));
for (var i=0; i < this.getBases$().size$(); i++) {
var j1=this.getBases$().get$I(i).getIndex$();
{
var j0=j1 - 1;
var j2=j1 + 1;
var p0=Clazz.new_($I$(5).c$$D$D,[0.0, 0.0]);
var p1=Clazz.new_($I$(5).c$$D$D,[0.0, 0.0]);
var p2=Clazz.new_($I$(5).c$$D$D,[0.0, 0.0]);
if (i == 0) {
if (i == this.getBases$().size$() - 1) {
p1=realCoords[j1];
p0=Clazz.new_($I$(5).c$$D$D,[p1.x + scaleFactor * this.getRadius$(), p1.y]);
p2=Clazz.new_($I$(5).c$$D$D,[p1.x - scaleFactor * this.getRadius$(), p1.y]);
} else {
p1=realCoords[j1];
p2=realCoords[j2];
...
var v1=Clazz.new_($I$(5).c$$D$D,[(p2.x - p1.x) / dist1, (p2.y - p1.y) / dist1]);
var vn1=Clazz.new_($I$(5).c$$D$D,[v1.y, -v1.x]);
var dist2=p1.distance$java_awt_geom_Point2D(p0);
var v2=Clazz.new_($I$(5).c$$D$D,[(p1.x - p0.x) / dist2, (p1.y - p0.y) / dist2]);
var vn2=Clazz.new_($I$(5).c$$D$D,[v2.y, -v2.x]);
var h=(Clazz.new_($I$(5).c$$D$D,[vn2.x - vn1.x, vn2.y - vn1.y]).distance$java_awt_geom_Point2D(Clazz.new_($I$(5).c$$D$D,[0, 0])) / 2.0);
var vn=Clazz.new_($I$(5).c$$D$D,[(vn1.x + vn2.x) / 2.0, (vn1.y + vn2.y) / 2.0]);
var D=vn.distance$java_awt_geom_Point2D(Clazz.new_($I$(5).c$$D$D,[0.0, 0.0]));
vn.x /= D;
vn.y /= D;
var nnorm=(D + h * h / D);
var nnormF=nnorm;
var nnormB=nnorm;
var interForward=Clazz.new_($I$(5).c$$D$D,[p1.x + nnormF * scaleFactor * this.getRadius$() * vn.x , p1.y + nnormF * scaleFactor * this.getRadius$() * vn.y ]);
var interBackward=Clazz.new_($I$(5).c$$D$D,[p1.x - nnormB * scaleFactor * this.getRadius$() * vn.x , p1.y - nnormB * scaleFactor * this.getRadius$() * vn.y ]);
if (pointList.size$() > 0) {
...
pointList.addLast$TE(interForward);
pointList.addFirst$TE(interBackward);
} else {
pointList.addFirst$TE(interForward);
pointList.addLast$TE(interBackward);
}} else {
pointList.addLast$TE(interForward);
pointList.addFirst$TE(interBackward);
}}}
...
var apoint=Clazz.new_($I$(5).c$$D$D,[mid.x + 1.0, mid.y]);
...
apoint=Clazz.new_($I$(5).c$$D$D,[mid.x + 1.0, mid.y]);
