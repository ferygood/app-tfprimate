...
var C$=Clazz.newClass(P$, "BioResolver", null, null, 'java.util.Comparator');
C$.htGroup=null;
C$.types=null;
C$.mytypes=null;
C$.htPdbBondInfo=null;
C$.pdbBondInfo=null;
C$.pdbHydrogenCount=null;
C$.group3Count=0;
C$.predefinedGroup1Names=null;
C$.group3NameCount=0;
C$.predefinedGroup3Names=null;
...
return Clazz.new_($I$(5).c$$org_jmol_modelset_ModelSet$I$I$S$java_util_Properties$java_util_Map,[this.ms, modelIndex, trajectoryBaseIndex, jmolData, modelProperties, modelAuxiliaryInfo]);
});
...
this.bsAddedHydrogens=Clazz.new_($I$(11));
this.bsAtomsForHs=Clazz.new_($I$(11));
this.htBondMap=Clazz.new_($I$(1));
this.htGroupBonds=Clazz.new_($I$(1));
this.hNames=Clazz.array(String, [3]);
this.vAB=Clazz.new_($I$(12));
this.vNorm=Clazz.new_($I$(12));
this.plane=Clazz.new_($I$(13));
});
...
for (var i=0; i < nH; i++) this.ms.addAtom$I$org_jmol_modelset_Group$I$S$S$I$I$I$javajs_util_P3$F$javajs_util_V3$I$F$F$F$javajs_util_Lst$Z$B$javajs_util_BS(a.mi, a.group, 1, "H", null, 0, a.getSeqID$(), 0, xyz, NaN, null, 0, 0, 1, 0, null, isHetero, ($b$[0] = 0, $b$[0]), null).delete$javajs_util_BS(null);
...
this.htGroupBonds.put$TK$TV(group3, $I$(15).TRUE);
for (var i=0; i < bondInfo.length; i++) {
if (bondInfo[i] == null ) continue;
if (bondInfo[i][1].charAt$I(0) == "H") this.htBondMap.put$TK$TV(group3 + "." + bondInfo[i][0] , bondInfo[i][1]);
 else this.htBondMap.put$TK$TV(group3 + ":" + bondInfo[i][0] + ":" + bondInfo[i][1] , bondInfo[i][2]);
}
}, p$1);
...
var htAtoms=Clazz.new_($I$(1));
var iterAtom=adapter.getAtomIterator$O(model);
while (iterAtom.hasNext$())htAtoms.put$TK$TV(iterAtom.getAtomName$(), iterAtom.getXYZ$());
...
$I$(16).sort$TTA$java_util_Comparator(bondInfo, this);
var t;
for (var i=0; i < n; ) {
t=bondInfo[i];
var a1=t[0];
...
$I$(17).getPlaneThroughPoints$javajs_util_T3$javajs_util_T3$javajs_util_T3$javajs_util_V3$javajs_util_V3$javajs_util_P4(htAtoms.get$O(name1), htAtoms.get$O(name), htAtoms.get$O(name2), this.vNorm, this.vAB, this.plane);
var d=$I$(17).distanceToPlane$javajs_util_P4$javajs_util_T3(this.plane, htAtoms.get$O(bondInfo[pt][1])) * factor;
...
Clazz.newMeth(C$, ['compare$SA$SA','compare$','compare$TT$TT'], function (a, b) {
...
$I$(18).info$S("Error adding H atoms to " + gName + g.getResno$() + ": " + pts[i].length + " atoms should not be added to " + aName );
continue;
...
d=$I$(17).distanceToPlane$javajs_util_P4$javajs_util_T3($I$(17).getPlaneThroughPoints$javajs_util_T3$javajs_util_T3$javajs_util_T3$javajs_util_V3$javajs_util_V3$javajs_util_P4(atom1, atom, atom2, this.vNorm, this.vAB, this.plane), pts[i][0]) * factor;
break;
}
if (pt < 0) {
$I$(18).info$S("Error adding H atoms to " + gName + g.getResno$() + ": expected to only need 1 H but needed 2" );
hName1=hName2="H";
} else if (d < 0 ) {
...
var bsBondsDeleted=Clazz.new_($I$(11));
...
this.ms.msInfo.put$TK$TV("bsDeletedAtoms", bsDeletedAtoms);
...
var key=Clazz.new_($I$(4)).append$S(g.getGroup3$());
key.append$S(":");
...
htKeysUsed.put$TK$TV(skey, $I$(15).TRUE);
...
if (key.indexOf$S(":") < 0) {
htKeysUsed.put$TK$TV(key, $I$(15).TRUE);
continue;
}var value=this.htBondMap.get$O(key);
$I$(18).info$S("bond " + key + " was not used; order=" + value );
if (this.htBondMap.get$O(key).equals$O("1")) {
htKeysUsed.put$TK$TV(key, $I$(15).TRUE);
continue;
}}
var htKeysBad=Clazz.new_($I$(1));
for (var key, $key = this.htBondMap.keySet$().iterator$(); $key.hasNext$()&&((key=($key.next$())),1);) {
if (htKeysUsed.get$O(key) != null ) continue;
htKeysBad.put$TK$TV(key.substring$I$I(0, key.lastIndexOf$S(":")), this.htBondMap.get$O(key));
}
...
if (Clazz.instanceOf(previous, "org.jmol.modelsetbio.AminoMonomer")) return Clazz.new_($I$(22).c$$org_jmol_modelsetbio_MonomerA$I,[monomers, pt0]);
if (Clazz.instanceOf(previous, "org.jmol.modelsetbio.AlphaMonomer")) return Clazz.new_($I$(23).c$$org_jmol_modelsetbio_MonomerA$I,[monomers, pt0]);
if (Clazz.instanceOf(previous, "org.jmol.modelsetbio.NucleicMonomer")) return Clazz.new_($I$(24).c$$org_jmol_modelsetbio_MonomerA,[monomers]);
if (Clazz.instanceOf(previous, "org.jmol.modelsetbio.PhosphorusMonomer")) return Clazz.new_($I$(25).c$$org_jmol_modelsetbio_MonomerA,[monomers]);
if (Clazz.instanceOf(previous, "org.jmol.modelsetbio.CarbohydrateMonomer")) return Clazz.new_($I$(26).c$$org_jmol_modelsetbio_MonomerA,[monomers]);
$I$(18).error$S("Polymer.allocatePolymer() ... no matching polymer for monomor " + previous);
...
if (this.bsAssigned == null ) this.bsAssigned=Clazz.new_($I$(11));
...
C$.htSpecialAtoms=Clazz.new_($I$(1));
for (var i=C$.specialAtomNames.length; --i >= 0; ) {
var specialAtomName=C$.specialAtomNames[i];
if (specialAtomName != null ) C$.htSpecialAtoms.put$TK$TV(specialAtomName, Byte.valueOf$B(($b$[0] = (i|0), $b$[0])));
}
}var boxedAtomID=C$.htSpecialAtoms.get$O(name);
return ($b$[0] = (boxedAtomID == null  ? (0|0) : (boxedAtomID.byteValue$()|0)), $b$[0]);
});

Clazz.newMeth(C$, 'getPdbBondInfo$S$Z', function (group3, isLegacy) {
if (C$.htPdbBondInfo == null ) C$.htPdbBondInfo=Clazz.new_($I$(1));
...
C$.htPdbBondInfo.put$TK$TV(group3, info);
return info;
}, p$1);
...
C$.htGroup.put$TK$TV("==" + C$.predefinedGroup1Names[i], Short.valueOf$H(($s$[0] = pt, $s$[0])));
}
...
if (C$.group3NameCount == $I$(3).group3Names.length) $I$(3).group3Names=$I$(27).doubleLengthS$SA($I$(3).group3Names);
var groupID=($s$[0]=C$.group3NameCount,C$.group3NameCount=(++$s$[0],$s$[0]),--$s$[0],$s$[0]);
$I$(3).group3Names[groupID]=group3;
C$.htGroup.put$TK$TV(group3, Short.valueOf$H(groupID));
return groupID;
}, 1);
