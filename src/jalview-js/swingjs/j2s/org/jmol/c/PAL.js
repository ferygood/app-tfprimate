...
var C$=Clazz.newClass(P$, "PAL", null, 'Enum');
C$.argbsCpkRasmol=null;
C$.argbsCpk=null;
...
C$.$init$.apply(this);
this.$name=name;
this.id=($b$[0] = (id|0), $b$[0]);
}, 1);

Clazz.newMeth(C$, 'pidOf$O', function (value) {
return ($b$[0] = (Clazz.instanceOf(value, "org.jmol.c.PAL") ? ((value).id|0) : (Clazz.instanceOf(value, "java.lang.Byte") ? ((value).byteValue$()|0) : (C$.UNKNOWN.id|0)|0)), $b$[0]);
}, 1);
...
if (paletteName.indexOf$I("_") < 0) for (var item, $item = 0, $$item = C$.values$(); $item<$$item.length&&((item=($$item[$item])),1);$item++) if (paletteName.equalsIgnoreCase$S(item.$name)) return $b$[0] = item.id, $b$[0];

return ($b$[0] = (paletteName.indexOf$S("property_") == 0 ? (C$.PROPERTY.id|0) : (C$.UNKNOWN.id|0)), $b$[0]);
}, 1);
