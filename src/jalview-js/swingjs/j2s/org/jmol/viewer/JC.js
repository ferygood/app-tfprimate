(function(){var P$=Clazz.newPackage("org.jmol.viewer"),I$=[[0,'java.util.Hashtable','javajs.util.V3','org.jmol.util.Elements','javajs.util.PT','org.jmol.util.Logger','javajs.util.SB']],$I$=function(i){return I$[i]||(I$[i]=Clazz.load(I$[0][i]))};
var C$=Clazz.newClass(P$, "JC");
C$.ruleNames=null;
C$.databaseArray=null;
C$.databases=null;
C$.macros=null;
C$.version=null;
C$.majorVersion=null;
...
C$.altArgbsCpk=null;
C$.argbsFormalCharge=null;
C$.argbsRwbScale=null;
C$.FORMAL_CHARGE_COLIX_RED=0;
C$.PARTIAL_CHARGE_COLIX_RED=0;
C$.PARTIAL_CHARGE_RANGE_SIZE=0;
C$.argbsRoygbScale=null;
C$.predefinedVariable=null;
C$.predefinedStatic=null;
C$.shapeClassBases=null;
C$.LABEL_DEFAULT_OFFSET=0;
C$.echoNames=null;
C$.hAlignNames=null;
C$.READER_NOT_FOUND=null;
...
C$.databases=Clazz.new_($I$(1));
{
for (var i=0; i < C$.databaseArray.length; i+=2) C$.databases.put$TK$TV(C$.databaseArray[i].toLowerCase$(), C$.databaseArray[i + 1]);

};
C$.macros=Clazz.array(String, -1, ["aflow", "http://aflowlib.mems.duke.edu/users/jmolers/jmol/spt/AFLOW.spt", "AFLOW macros", "bz", "http://aflowlib.mems.duke.edu/users/jmolers/jmol/spt/bz.spt", "Brillouin Zone/Wigner-Seitz macros"]);
C$.version=null;
C$.date=null;
C$.versionInt=0;
{
...
C$.date=(tmpDate != null  ? tmpDate : "(Unknown_date)");
var v=-1;
try {
var s=C$.version;
var major="";
...
C$.center=$I$(2).new3$F$F$F(0, 0, 0);
C$.axisX=$I$(2).new3$F$F$F(1, 0, 0);
C$.axisY=$I$(2).new3$F$F$F(0, 1, 0);
C$.axisZ=$I$(2).new3$F$F$F(0, 0, 1);
C$.axisNX=$I$(2).new3$F$F$F(-1, 0, 0);
C$.axisNY=$I$(2).new3$F$F$F(0, -1, 0);
C$.axisNZ=$I$(2).new3$F$F$F(0, 0, -1);
C$.unitAxisVectors=Clazz.array($I$(2), -1, [C$.axisX, C$.axisY, C$.axisZ, C$.axisNX, C$.axisNY, C$.axisNZ]);
...
C$.FORMAL_CHARGE_COLIX_RED=$I$(3).elementSymbols.length + C$.altArgbsCpk.length;
...
if (newname != name) $I$(5).info$S("JC.fixProtocol " + name + " --> " + newname );
return newname;
}, 1);

Clazz.newMeth(C$, 'getMacroList$', function () {
var s=Clazz.new_($I$(6));
for (var i=0; i < C$.macros.length; i+=3) s.append$S(C$.macros[i]).append$S("\t").append$S(C$.macros[i + 1]).append$S("\t").append$S(C$.macros[i + 1]).append$S("\n");

return s.toString();
}, 1);

Clazz.newMeth(C$, 'getMacro$S', function (key) {
for (var i=0; i < C$.macros.length; i+=3) if (C$.macros[i].equals$O(key)) return C$.macros[i + 1];
