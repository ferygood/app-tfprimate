...
this.htDefs=Clazz.new_($I$(2));
this.m=Clazz.new_($I$(3));
this.models=Clazz.new_($I$(4));
this.resources=Clazz.new_($I$(4));
this.modifiers=Clazz.new_($I$(4));
this.htNodes=Clazz.new_($I$(2));
this.cylinderMatrix=Clazz.new_($I$(3));
...
sb.append$S(P$.___Exporter.round$D(pt.x)).append$S(" ").append$S(P$.___Exporter.round$D(pt.y)).append$S(" ").append$S(P$.___Exporter.round$D(pt.z)).append$S(" ");
}, p$1);
...
this.htDefs.put$TK$TV(key, $I$(7).TRUE);
...
this.resources.append$S("MATERIAL_OPACITY " + P$.___Exporter.opacityFractionalFromColix$H(colix) + "\n" );
this.resources.append$S("}}\n");
}, p$1);
...
var a=$I$(8).getQuaternionFrame$javajs_util_P3$javajs_util_T3$javajs_util_T3(center, points[1], points[3]).toAxisAngle4f$();
...
if (v == null ) {
v=Clazz.new_($I$(9));
this.htNodes.put$TK$TV(key, v);
p$1.addShader$S$H.apply(this, [key, colix]);
}v.addLast$TV(p$1.getParentItem$S$javajs_util_M4.apply(this, ["Jmol", sphereMatrix]));
}, p$1);
...
var vertexCount=$I$(10).getVertexCount$I(2);
var f=$I$(10).getFaceVertexes$I(2);
...
var vertexes=Clazz.array($I$(11), [vertexCount]);
for (var i=0; i < vertexCount; i++) vertexes[i]=$I$(10).getVertexVector$I(i);
...
v.addLast$TV(p$1.getParentItem$S$javajs_util_M4.apply(this, ["Jmol", this.cylinderMatrix]));
radius *= 0.95;
}
return true;
});
...
return;
}var sbFaceCoordIndices=this.sbTemp=Clazz.new_($I$(4));
...
var sbFaceNormalIndices=this.sbTemp=Clazz.new_($I$(4));
var vNormals=null;
if (normals != null ) {
vNormals=Clazz.new_($I$(9));
map=this.getNormalMap$javajs_util_T3A$I$javajs_util_BS$javajs_util_Lst(normals, nVertices, null, vNormals);
this.outputIndices$IAA$IA$I$javajs_util_BS$I(indices, map, nPolygons, bsPolygons, faceVertexMax);
}map=null;
var sbColorIndexes=Clazz.new_($I$(4));
if (colorList != null ) {
...
vNormals=null;
}var sbColors=Clazz.new_($I$(4));
var nColors=0;
if (colorList != null ) {
nColors=colorList.size$();
for (var i=0; i < nColors; i++) {
var c=colorList.get$I(i).shortValue$();
sbColors.append$S(this.rgbFractionalFromColix$H(c)).append$S(" ").append$S(P$.___Exporter.translucencyFractionalFromColix$H(c)).append$S(" ");
}
...
var m=P$.___Exporter.getConeMesh$javajs_util_P3$javajs_util_M3$H(null, null, 0);
...
var colix=$I$(12).getColix$I(argb);
...
var normals=Clazz.array($I$(11), -1, [this.tempV2, this.tempV2, this.tempV2]);
